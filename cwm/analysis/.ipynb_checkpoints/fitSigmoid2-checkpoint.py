...
class fitSigmoid:    
    ...
    def h0_fromPercentile(self, per=0.95):    
        ...
        dh = dx*self.h0_max
        return self._invRescale_h0(x), dh
    ...
    def plot(self, h0_list, p, savePath=None):
        ...
        ax.errorbar(h0_arr, p_inter, xerr=err, fmt='', color=colors[0], ls='none')
                 
        ax.scatter(h0_list, p, color='black')
        err = self.binomialError(p, self.injPerPoint)
        ax.errorbar(h0_list, p, yerr=err, fmt='', label='data', color='k', ls='none')
               
        h95, dh95 = self.h0_fromPercentile(per=0.95)
        p95 = 0.95
        x_highlighting = [0, h95, h95]
        y_highlighting = [p95, p95, -1]

        plt.plot(x_highlighting, y_highlighting, color='r', label=r'$h_{95}=$'+'{:.3e}'.format(h95)+r'$\pm$'+'{:.2f}%'.format(dh95/h95*100))
