...
class SearchParamGenerator:
    ...
    def __init__(self, model, freq_deriv_order=2):
        ...
        self.freq_param_name, self.freq_deriv_param_name = phase_param_name(
            freq_deriv_order
        )
        self.model = model
    ...
    def generate_parameter_table(
        self, freq, df, n_f1, n_f2, n_f3=1, n_f4=1, bandwidth=1.0
    ):
        """
        ...
        dtype_list = [
            (key, ">f8") for key in (self.freq_param_name + self.freq_deriv_param_name)
        ]
            ...
            for j in range(n_f1):
                ...
                for k in range(n_f2):
                    ...
                    if (
                        j == (n_f1 - 1)
                        and k == 0
                        and isinstance(self.model, PowerLawModel)
                    ):
                        f2_min = 0.0
                        f2_band = f2_max
                    ...
                    for p in range(n_f3):
                        ...
                        for m in range(n_f4):
                            ...
                            if self.freq_deriv_order >= 3:
                                data[idx]["f3dot"], data[idx]["df3dot"] = (
                                    f3_min,
                                    f3_band,
                                )
                            if self.freq_deriv_order >= 4:
                                data[idx]["f4dot"], data[idx]["df4dot"] = (
                                    f4_min,
                                    f4_band,
                                )
    ...
    def generate_parameters(
        ...
        while current_freq < f0_max:
            ...
            table = self.generate_parameter_table(
                current_freq, df, n_f1, n_f2, n_f3, n_f4, bandwidth=chunk_bw
            )
