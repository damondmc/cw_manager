...
def main():
    parser = argparse.ArgumentParser(description='Calculate 95% Upper Limit via Injection.')
    parser.add_argument('--config_file', type=str, help='Path to main config yaml', default='/home/hoitim.cheung/galacticCenter/config/config.yaml')
    parser.add_argument('--target_file', type=str, help='Path to target config yaml', default='/home/hoitim.cheung/galacticCenter/config/GalacticCenter.yaml')
    parser.add_argument('--taskname', type=str, default='GalacticCenter_upperlimit_TCoh5_O2_97Hz') 
    ...
    parser.add_argument('--df_grid', type=float, nargs='+', default=[1e-6, 1e-13, 1e-20], help='Frequency grid spacing for searches.')
    parser.add_argument('--inj_freq_deriv_order', type=int, default=4)
    parser.add_argument('--sft_files', type=str, default='./')
    parser.add_argument('--metric_file', type=str, default='./Start1368970000_TCoh432000_N107_Spin2.fts') 
    parser.add_argument('--n_inj', type=int, default=200)
    parser.add_argument('--num_toplist', type=int, default=1, help='Number of top candidates to consider for efficiency calculation.')
    ...
    if len(df_grid) != len(freq_deriv_names):
        raise ValueError(f"Length of df_grid ({len(df_grid)}) does not match number of frequency derivative names ({len(freq_deriv_names)})")
    ...
    injection_generator = InjectionParamGenerator(model=model, ref_time=ref_time, f0_band=f0_band)
    ...
    for factor in factors:
        ...
        logging.info(f"Iteration took {time.time()-loop_t0:.2f}s | Efficiency: {eff:.2%}")
    ...
    while True:
        ...
        logging.info(f"Refinement Step | Current h95: {h95:.3e} | Uncertainty: {dx*100:.2f}%")
    ...
    with fits.open(outlier_file_path, mode='update') as hdul:
        ...
        col_pcov = fits.Column(name='pcov', format=f'{len(fitter.pcov.flatten())}E', array=fitter.pcov.flatten()) 
