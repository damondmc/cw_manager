...
def injectionFollowUp(fm, rm, target, obsDay, freq, sftFiles, 
    ...
    #if new_freqDerivOrder >=3:
    #    print(sp['100'].data['f3dot'])
    #    print(sp['100'].data['df3dot'])
    #    print(ip['100'].data['f3dot'])
    ...
    # analyze the result
    ratio = 0
    outlierFilePath = rm.writeFollowUpResult(old_cohDay, new_cohDay, freq, numTopList=numTopList, old_stage=old_stage, new_stage=new_stage, old_freqDerivOrder=old_freqDerivOrder, new_freqDerivOrder=new_freqDerivOrder, ratio=ratio, workInLocalDir=workInLocalDir, inj=True, cluster=cluster)
        ...
        for chunk_index, chunk in enumerate(chunked_iterable(search_params, chunk_size)):
            ...
            if chunk_count != 1:
                ...
                outlierFilePath = rm.writeFollowUpResult(
                    old_cohDay, new_cohDay, freq, numTopList=numTopList, old_stage=old_stage, 
                    new_stage=new_stage, old_freqDerivOrder=old_freqDerivOrder, 
                    new_freqDerivOrder=new_freqDerivOrder, ratio=mean2F_ratio, 
...
def _realFollowUp(rm, sp, target, obsDay, freq, sftFiles, mean2F_ratio,
                 old_cohDay, old_freqDerivOrder, old_stage, 
                 new_cohDay, new_freqDerivOrder, new_stage, 
                 numTopList, extraStats, num_cpus, setup, 
                 cluster, workInLocalDir, saveIntermediate=False):
